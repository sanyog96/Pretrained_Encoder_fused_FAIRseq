...
def _make_dataset(
    ...
    if args.dataset_impl == "raw":
        ...
        _make_binary_dataset(
            vocab, input_prefix, output_prefix, lang, num_workers, args, append_eos = append_eos
        )


def _make_all(lang, vocab, args, append_eos = True):
    if args.trainpref:
        _make_dataset(
            vocab, args.trainpref, "train", lang, args=args, num_workers=args.workers, append_eos = append_eos
        )
    if args.validpref:
        for k, validpref in enumerate(args.validpref.split(",")):
            outprefix = "valid{}".format(k) if k > 0 else "valid"
            _make_dataset(
                vocab, validpref, outprefix, lang, args=args, num_workers=args.workers, append_eos = append_eos
            )
    if args.testpref:
        for k, testpref in enumerate(args.testpref.split(",")):
            outprefix = "test{}".format(k) if k > 0 else "test"
            _make_dataset(
                vocab, testpref, outprefix, lang, args=args, num_workers=args.workers, append_eos = append_eos
            )
...
def main(args):
    ...
    _make_all(args.source_lang, src_dict, args, append_eos = False)
    if target:
